...
- [ ] **Input / Output**:  
  Audio is input via a 1/8-inch aux cable and output through a pair of speakers in the digital lab.

- [ ] **Left & Right Channels**:  
  Audio is correctly separated for left and right channels (if audio source plays stereo audio).
...
Because the filtering is duplicated, we duplicate the filtering hardware. Each channel has 3 cascading filters (low, mid, high), which consist of 5 biquads. That means that we do $3 x 5 x 2 = 30$ multiplies per sample. These are 16 bit multiples, and the FPGA has 8 16x16 DSP slices. That means that as long as we run the FPGA at least 3.75 times the sampling frequency of the ADC/DAC then we can fit in all the multiplies. For a sampling rate of 96 KHz, this will be easy as the FGPA can run on order of MHz.